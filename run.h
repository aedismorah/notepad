...
void run(char* text, HWND hwnd)
{
	...
	for(char* line : lines)
	{
		...
		if(!strcmp(words[0], "create"))
		{
			...
			GetCurrentDirectory(0, filename), MessageBox(0, filename ,"Error!", 0), filename = (char*)realloc(filename, sizeof(filename) + sizeof(append) + 1);
			strcat(filename, append), SaveFile(GetDlgItem(hwnd, IDC_MAIN_TEXT), (LPSTR)filename);
			free(append);
			//free(filename);
		}
	}
